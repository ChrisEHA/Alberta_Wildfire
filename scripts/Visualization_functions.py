...
def plot_multiple_ROC(y_true, y_prediction_proba, names):
    """
    Plot ROC curves for multiple predictions on the same plot. Optionally handles different true labels for each prediction.

    Parameters:
    - y_true: array-like of shape (n_samples,) or list of array-like shapes (n_samples,)
             - True binary labels. Can be a single array or a list of arrays.
    ...
    # Handle both single array y_true and list of arrays y_true
    if not isinstance(y_true, list):
        y_true = [y_true] * len(y_prediction_proba)  # Repeat the single y_true for each prediction

    for y_true_single, y_pred, name in zip(y_true, y_prediction_proba, names):
        # Compute ROC curve and ROC area for each class
        fpr, tpr, _ = roc_curve(y_true_single, y_pred)
        roc_auc = auc(fpr, tpr)
...
## Functions for plotting predictions on map ##
...
def print_metrics(y_true,y_pred):
    ...
    print("Validation set accuracy:", accuracy_score(y_true, y_pred))
